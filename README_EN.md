...
| Presigned direct upload | Browser uploads straight to R2 (up to 5GB per file), no server relay |
| Media pipeline | Images → multi-tier WebP, video → multi-bitrate HLS, music → MP3 128k/320k, fully automatic |
...
| Secure by default | JWT + login rate limit, upload size/type allowlist, fail-closed config, secret isolation |
...
- [ ] Real-device mobile HLS matrix (iOS Safari / Android Chrome)
- [ ] Further admin build-page polish
- [ ] Concurrent/resumable uploads
