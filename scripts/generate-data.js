...
import { CONTENT_DIR, DIST_DIR, ensureDir, computeScore, writeJSON, readJSON, readFile, log, warn, r2MediaUrl } from './utils.js';
...
async function parsePosts(site) {
  ...
  for (const dir of postDirs) {
    ...
    // Collect music metadata
    const music = [];
    const musicDir = path.join(postPath, 'music');
    if (await fs.pathExists(musicDir)) {
      // Read music-meta.json if it exists (generated by extract-music-meta.js)
      const musicMetaPath = path.join(musicDir, 'music-meta.json');
      const musicMeta = await readJSON(musicMetaPath);

      if (musicMeta && Array.isArray(musicMeta)) {
        for (const track of musicMeta) {
          const trackSources = {};
          if (track.sources) {
            for (const [quality, file] of Object.entries(track.sources)) {
              trackSources[quality] = r2MediaUrl(slug, 'music', file);
            }
          }
          const trackCover = track.cover
            ? r2MediaUrl(slug, 'music', track.cover)
            : '';
          music.push({
            file: track.file,
        ...
        // Fallback: scan music directory directly
        const files = (await fs.readdir(musicDir)).sort();
        const musicExts = ['mp3', 'flac', 'wav', 'aiff', 'm4a', 'ogg'];
        for (const f of files) {
          const ext = path.extname(f).toLowerCase().slice(1);
          if (!musicExts.includes(ext)) continue;
          const base = path.parse(f).name;
          // Skip processed variants — only list the "main" file
          if (base.includes('-320k') || base.includes('-128k')) continue;
          music.push({
            file: base,
            title: base,
            artist: '',
            ...
            waveform: [],
            sources: {
              '320k': r2MediaUrl(slug, 'music', `${f}`),
              '128k': r2MediaUrl(slug, 'music', `${base}-128k.mp3`),
            },
          });
        }
      }
    ...
    // Determine media types present
    const mediaTypes = ['article'];
    if (photos.length > 0) mediaTypes.push('photo');
    if (videos.length > 0) mediaTypes.push('video');
    if (music.length > 0) mediaTypes.push('music');
